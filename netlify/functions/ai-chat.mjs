...
// ZIP code prefix to state code mapping (first 3 digits)
const ZIP_PREFIX_TO_STATE = {
  ...
  '200':'DC','201':'VA','202':'DC','203':'DC','204':'DC','205':'DC','206':'MD','207':'MD','208':'MD','209':'MD','210':'MD','211':'MD','212':'MD','214':'MD','215':'MD','216':'MD','217':'MD','218':'MD','219':'MD',
  '220':'VA','221':'VA','222':'VA','223':'VA','224':'VA','225':'VA','226':'VA','227':'VA','228':'VA','229':'VA','230':'VA','231':'VA','232':'VA','233':'VA','234':'VA','235':'VA','236':'VA','237':'VA','238':'VA','239':'VA','240':'VA','241':'VA','242':'VA','243':'VA','244':'VA','245':'VA','246':'WV','247':'WV','248':'WV','249':'WV','250':'WV','251':'WV','252':'WV','253':'WV','254':'WV','255':'WV','256':'WV','257':'WV','258':'WV','259':'WV','260':'WV','261':'WV','262':'WV','263':'WV','264':'WV','265':'WV','266':'WV','267':'WV','268':'WV',
...
export default async (req, context) => {
  ...
  try {
    ...
    if (/^\d{5}$/.test(zip)) {
      try {
        ...
        if (zipRes.ok) {
          ...
          zipDataFound = true;
          // Also capture state from ZIP data if available
          if (zipData.state && /^[A-Z]{2}$/.test(zipData.state)) {
            resolvedStateCode = zipData.state;
          }
        }
      } catch (e) {
    ...
    // Determine state code: from ZIP data, from ZIP prefix lookup, or from client parameter
    if (!resolvedStateCode && /^\d{5}$/.test(zip)) {
      resolvedStateCode = getStateFromZip(zip) || '';
    }
    if (!resolvedStateCode && /^[A-Z]{2}$/i.test(stateParam)) {
      resolvedStateCode = stateParam.toUpperCase();
    }

    // Fetch state-level data as fallback when ZIP data is unavailable
    if (resolvedStateCode && !zipDataFound) {
      try {
    ...
    if (zip && /^\d{5}$/.test(zip)) {
      if (zipDataFound) {
        ...
        zipInstruction = `\n${stateDataSection}\nIMPORTANT: We do not have ZIP-specific rebate data for ZIP ${zip}, but the state-level data above contains real programs, incentives, and utility rebates for this user's state. Use this data to provide state-specific guidance. Reference actual program names, amounts, and utility programs from the data. Tell the user: "While I don't have rebate data specific to your exact ZIP code, here's what's available in your state." Also suggest they contact their local utility for additional local programs.\n`;
      } else {
